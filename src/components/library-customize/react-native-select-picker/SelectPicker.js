    ...
    return (
      <View style={[styles.wrap, { height: wrapHeight, width: wrapWidth }, wrapStyle]}>
        ...
        >
          ...
          {
            data.map((item, index) => {
              return <Item fontSize={fontSize} selectedIndex={selectedIndex} fontColor={fontColor} style={{ alignItems: 'center', justifyContent: 'center', height: itemHeight, width: wrapWidth }} item={item} index={index} key={index} />
            })
          }
          <View style={{ height: maskItemHeight, width: wrapWidth }} ></View>
        </Animated.View>
...
class Item extends React.Component {
  render() {
    var { item, style, fontColor, fontSize, selectedIndex, index } = this.props
    return (
      <View style={style}>
        <Text style={[selectedIndex == index ? { color: "#3498DB" } : { color: fontColor }, { textAlign: 'center', fontSize: fontSize }]}>{item}</Text>
      </View>
    )
  }
}
